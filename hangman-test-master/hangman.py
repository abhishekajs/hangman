    ...
    * Display partial word guessed by the user and use underscore in place of not guess word
    ...
    while True:
        if remaining_lives>0:
            ...
            if hint==1:
                temp=input("Need hint? If yes then type 'hint' otherwise press 'Enter'.")
                if temp.lower()=="hint":
                    hint-=1
                    for i in available_letters:
                        if i in secret_word:
                            ...
                            break
                    continue
            guess = input("Guess the letter of the world : ")
            letter = guess.lower()
            ...
            print(IMAGES[-1])
            print("Next time bro, come with more practice and save the man.")
            break
